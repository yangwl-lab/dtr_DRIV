...
)
{
  ...
  for (int iter = 0; iter < max_iter; iter++)
  {
    ...
    for (int i = 0; i < n; i++)
    {
      for (int j = 0; j < k; j++)
      {
        if (j == 0)
        {
          dLam[j] = dLam[j] + Yt(i, j) * exp(int_D(i, j) * betaD) * (dNt(i, j) - D_status(i, j)*betaD*stime[j] - ConfoundingPart(i, j)*stime[j])/ SY[j];
          dLam_dbetaD[j] = dLam_dbetaD[j]+Yt(i, j)*int_D(i, j)*exp(int_D(i, j)*betaD)*(dNt(i,j) - (D_status(i,j)*betaD + ConfoundingPart(i, j))*stime[j])/SY[j] - 
            Yt(i, j)*exp(int_D(i, j)*betaD)*D_status(i,j)*stime[j]/SY[j] - 
            dSY[j]*Yt(i, j) * exp(int_D(i, j) * betaD) * (dNt(i, j) - D_status(i, j)*betaD*stime[j] - ConfoundingPart(i, j)*stime[j])/ (SY[j] * SY[j]);
        } else
        {
          dLam[j] = dLam[j] + Yt(i, j) * exp(int_D(i, j) * betaD) * (dNt(i, j) - D_status(i, j)*betaD*(stime[j] - stime[j-1]) -
            ConfoundingPart(i, j)*(stime[j] - stime[j-1]))/ SY[j];
          dLam_dbetaD[j] = dLam_dbetaD[j]+Yt(i, j)*int_D(i, j)*exp(int_D(i, j)*betaD)*(dNt(i,j) - (D_status(i,j)*betaD + ConfoundingPart(i, j))*(stime[j] - stime[j-1]))/SY[j] - 
            Yt(i, j)*exp(int_D(i, j)*betaD)*D_status(i,j)*(stime[j] - stime[j-1])/SY[j] - 
            dSY[j]*Yt(i, j) * exp(int_D(i, j) * betaD) * (dNt(i, j) - D_status(i, j)*betaD*(stime[j] - stime[j-1])- ConfoundingPart(i, j)*(stime[j] - stime[j-1]))/ (SY[j] * SY[j]);
        }
      }
    }
    // printf("2\n");
    ...
    for (int i = 0; i < n; i++)
    {
      for (int j = 0; j < k; j++)
      {
        ...
        if (j == 0)
        {
          ...
          res[i] = res[i] + exp(int_D(i, j) * betaD) * (dNt(i, j)- Yt(i, j) * (dLam[j] + ConfoundingPart(i, j)*stime[j])) - int_expbetaD(i, j) * (IV[i] * betaD);
          dPhi[i] = dPhi[i] + (dNt(i, j) - Yt(i, j) * (dLam[j] + ConfoundingPart(i, j)*stime[j])) * int_D(i, j) * exp(betaD * int_D(i, j)) -
        ...
        else
        {
          ...
          res[i] = res[i] + exp(int_D(i, j) * betaD) * (dNt(i, j) - Yt(i, j)*(dLam[j]+ConfoundingPart(i, j)*(stime[j] - stime[j-1]))) - Yt(i, j-1) * int_expbetaD(i, j) * (D_status(i, j-1) * betaD);
          dPhi[i] = dPhi[i] + (dNt(i, j) - Yt(i, j) * (dLam[j]+ConfoundingPart(i, j)*(stime[j] - stime[j-1]))) * int_D(i, j) * exp(betaD * int_D(i, j)) -
            exp(betaD*int_D(i,j))*Yt(i,j)*dLam_dbetaD[j]-
  ...
  for (int i = 0; i < n; i++)
  {
    for (int j = 0; j < k; j++)
    {
      if (j == 0)
      {
        ave_hatsigma = ave_hatsigma + (IV_c[i]*exp(int_D(i, j) * betaD) *(dNt(i, j) - Yt(i, j) * (dLam[j] + ConfoundingPart(i, j)*stime[j])) - 
          IV_c[i] * int_expbetaD(i, j)* 1*(betaD* IV[i]))/n;
      } else
      {
        ave_hatsigma = ave_hatsigma + (IV_c[i]*exp(int_D(i, j) * betaD) *(dNt(i, j)- Yt(i, j) * (dLam[j] + ConfoundingPart(i, j)*(stime[j] - stime[j-1]))) - 
  ...
  for (int i = 0; i < n; i++)
  {
    for (int j = 0; j < k; j++)
    {
      ...
      if(j==0)
      {
        hatsigma_v[i] = hatsigma_v[i] + (IV_c[i]*exp(int_D(i, j) * betaD) *(dNt(i, j)- Yt(i, j) * (dLam[j] + ConfoundingPart(i, j)*stime[j])) - 
          IV_c[i] * int_expbetaD(i, j)* 1*(betaD*IV[i]));
        hatphi = hatphi + (IV_c[i] * int_D(i, j)* exp(int_D(i,j)*betaD)*(dNt(i, j) - Yt(i, j)*(dLam[j] + ConfoundingPart(i, j)*stime[j])) -
          IV_c[i] * int_dexpbetaD(i, j)*1*(betaD*IV[i]) -
          IV_c[i] * int_expbetaD(i, j)*1*IV[i]);
      } else 
      {
        hatsigma_v[i] = hatsigma_v[i] + IV_c[i]*exp(int_D(i, j) * betaD) *(dNt(i, j)- Yt(i, j) * (dLam[j] + ConfoundingPart(i, j)*(stime[j] - stime[j-1]))) - 
          IV_c[i] * int_expbetaD(i, j)* Yt(i, j-1)*(betaD*D_status(i, j-1));
        hatphi = hatphi + (IV_c[i] * int_D(i, j)* exp(int_D(i,j)*betaD)*(dNt(i, j)- Yt(i, j)*(dLam[j] + ConfoundingPart(i, j)*(stime[j] - stime[j-1]))) -
