...
sourceCpp("Function/integral/cf_integral.cpp")
...
F  = function(x, time, event , IV = IV, cv_IV_c ,
              Covariates, D_status , stime ,
              cv_beta , cv_dLam , max_iter ,
              tol , contraction , eta ){
  out = cf_integral(x, time = time, event = event, IV = IV, cv_IV_c = cv_IV_c,
                        Covariates = Covariates, D_status = D_status, stime = stime,
                        cv_beta = cv_beta, cv_dLam = cv_dLam, max_iter = max_iter,
                        tol = tol, contraction = contraction, eta = eta)
  return(out$fn)
}


# BasisFun: a list of functions
cfhaz_integral_cpp = function(init_parameters, time, event, IV, 
                            ...
                            contraction = 0.5, eta = 1e-4, nfolds = 10, seed = 5884419)
{
  ...
  out = nleqslv::nleqslv(0.1, F, time = time, event = event, IV = IV, cv_IV_c = cf_IV_c,
                   Covariates = b_Covariates, D_status = D_status, stime = stime,
                   cv_beta = cf_beta, cv_dLam = cf_dLam, max_iter = max_iter,
                   tol = tol, contraction = contraction, eta = eta)
